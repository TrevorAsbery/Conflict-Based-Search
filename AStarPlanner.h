...
//GOAL: is the same as vertex constraint only it means that after
//the given time step nothing can cross that location
enum ConstraintType {VERTEX, EDGE, GOAL};
...
struct AStarNode {
  ...
  AStarNode(int location, int g, int h, AStarNode* parent, int _timestep):
    location(location), g(g), h(h), parent(parent), timestep(_timestep) {}
};
...
private:
  ...
  bool check_constraints(int agent_id, AStarNode* next, const list<Constraint>& constraints);
